...
- **CodeBreaker/CodeBreakerModel/CodeBreakerApp.swift** *(修改)*: modelContainer 提升至 WindowGroup 层级，修复作用域过早问题
- **CodeBreaker/UI/CodeBreakerView.swift** *(修改)*: 旋转手势从角度增量改为绝对值映射（`abs(degrees) / 90 % count`），frame 高度微调 60→50 / 85→70
- **CodeBreaker/UI/GameChooser.swift** *(修改)*: NavigationSplitView 新增 `columnVisibility` 绑定状态
- **CodeBreaker/UI/GameList.swift** *(修改)*: onAppear 重构为 do/catch 错误处理 + 调试日志；GameSummary 重新包裹 NavigationLink 支持导航
...
第二轮提交：修复 modelContainer 作用域（从 GeometryReader 内提升至 WindowGroup 层级），旋转手势简化算法（角度增量 → 绝对值映射），NavigationSplitView 加入 columnVisibility 控制，GameList onAppear 添加完整的错误处理和调试输出。
